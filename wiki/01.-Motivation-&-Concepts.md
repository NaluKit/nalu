...
Parameters are also added to the route, so there is no need to create a place (like it is necessary in GWT Activities & Places). In case Nalu identifies a controller, it will inject the parameters (which are defined by the route definition inside the `@Controller`-annotation) into the controller.
...
| Clean URL Support                                          | No  | [Yes](https://github.com/NaluKit/nalu/wiki/06.-Application#clean-urls-hash-less-routing-since-v400) | 
...
### Composite
In Nalu you can separate a visual component into smaller components. This smaller pieces are called **Composites**. Composites have a controller and a component - similar to components, but can not be attached to route. 
...
Using [GWT](https://www.gwtproject.org/doc/latest/DevGuideMvpActivitiesAndPlaces.html) you have to create an activity and a place, containing the Tokenizer, for each screen.
...
### Component-Caching
Nalu supports [component](https://github.com/NaluKit/nalu/wiki/14.-Controllers-&-Composites#caching) and [composite](https://github.com/NaluKit/nalu/wiki/14.-Controllers-&-Composites#caching) caching. 
