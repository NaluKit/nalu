...
| Clean URL        | `http://localhost:8080/app/route`                | SEO, bookmarks, readable    | Needs server config        |
...
Nalu ususally uses the `DefailtCorePlugin`for hash-handlung. To enable the clean URL feature, you need to config the Applcaition annotation as mentioned above and change the Core-Plugin to `CleanUrlWithParameterHandlingCoreWebPlugin` by calling: `NaluCorePluginFactory.INSTANCE.registerPlugin(new CleanUrlWithParameterHandlingCoreWebPlugin());`.
...
- Nalu then uses the `uri` parameter as the start route.
