...
* **useHash**: if **true** Nalu uses a hash token, otherwise a hash-less token (Clean URLs). (default: **true**)

* **usingColonForParametersInUrl**: if **true** Nalu uses a ':' before a parameter inside the url (default: **false**)
...
| Clean URL        | `http://localhost:8080/app/route`                | SEO, bookmarks, readable    | Needs server config, server roundtrip |

### How to enable Clean URLs in Nalu

- Set `useHash = false` in your `@Application` annotation.
...
By default Nalu uses the `DefaultCorePlugin` for hash-handlung. To enable the clean URL feature, you need to config the Applcaition annotation as mentioned above and change the Core-Plugin to `CleanUrlWithParameterHandlingCoreWebPlugin` by calling: `NaluCorePluginFactory.INSTANCE.registerPlugin(new CleanUrlWithParameterHandlingCoreWebPlugin());`.
A great place to change set the default-behavior is in side the `onModuleLoad`-method of you application class just before calling the `run`-method.
...
- Nalu then uses the `uri` parameter as start route.
