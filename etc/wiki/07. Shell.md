...
To define a shell in Nalu, you have to use the `@Shells`-annotation inside the application interface.
...
* **applicationShell** to define the shell for the application

To tell Nalu, which shell is to be used, the name of the shell must be the first part of the route:

`/loginShell/login`

Nalu will look for a shell defined with the name 'loginShell' in the `@Shells`-annotation and add it to the viewport. In case that a shell is already existing, Nalu will remove the current shell and call the `onDetach`-method.
