...
The application shellCreator is the root view of the application. See [07.Shell](https://github.com/nalukit/nalu/wiki/07.-Shell) for more informations.

The Nalu GWT plugin provides two more base components. Besides the ```ÀbstractShell```-
class, there are two more base classes:

* **AbstractCompisteShell**: Base shellCreator class, that extends ```Compiste```
* **AbstractResizeCompositeShell**: Base shellCreator class, that extends ```ResizeCompiste```
...
To define a shellCreator in Nalu, you have to use the `@Shells`-annotation inside the application interface.
...
The code above will define two shells.

* **loginShell** to define the layout of the login page
* **applicationShell** to define the shellCreator for the application

To tell Nalu, which shellCreator is to be used, the name of the shellCreator must be the first part of the route:

`/loginShell/login`

Nalu will look for a shellCreator defined with the name 'loginShell' in the `@Shells`-annotation and add it to the viewport. In case that a shellCreator is already existing, Nalu will remove the current shellCreator and call the `onDetach`-method.
