...
* **hasHistory**: if **true** Nalu supports history, so the applicaiton can reload. Otherwise no history will be created and the url stays untouched. This might be useful, in case the application should never reload or will be used embedded inside another web side. (default: **true**) - since v1.2.0
