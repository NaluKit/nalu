 ...
 *
 */
package com.github.mvp4g.nalu.react.processor.scanner.validation;
...
public class ApplicationAnnotationValidator {
  ...
  public void validate()
    ...
    if (elementsWithApplicaitonAnnotation.size() == 0) {
      throw new ProcessorException("NaluReact-Processor: Missing NaluReact Application interface");
    }
    // at least there should only one Application annotation!
    if (elementsWithApplicaitonAnnotation.size() > 1) {
      throw new ProcessorException("NaluReact-Processor: There should be at least only one interface, that is annotated with @Application");
    }
  }
  ...
  public void validate(Element element)
    ...
    if (element instanceof TypeElement) {
      ...
      if (!typeElement.getKind()
                      .isInterface()) {
        throw new ProcessorException("NaluReact-Processor: @Application annotated must be used with an interface");
      }
      ...
      if (!this.processorUtils.extendsClassOrInterface(this.processingEnvironment.getTypeUtils(),
                                                       ...
                                                       this.processingEnvironment.getElementUtils()
                                                                                 ...
                                                                                 .asType())) {
        throw new ProcessorException("NaluReact-Processor: " + typeElement.getSimpleName()
                                                                      .toString() + ": @Application must implement IsApplication interface");
      }
    } else {
      throw new ProcessorException("NaluReact-Processor:" + "@Application can only be used on a type (interface)");
    }
  }
